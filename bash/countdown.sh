...
trap counterReset SIGINT
trap exitNow SIGQUIT
...
# When sigint is got then reset funciton is called
function counterReset {
	...
	# restarting count from maximum value
doCountdown | dialog --gauge "Remaining Time" 7 60
stty sane
} 

# when sigquit is found exitNow function is called
function exitNow {
	echo "You have found the secret to get out of this script"
	# to stop the count and exit
	exit
}
