...
#         host names, etc.
myHostname=$(hostname)
#lan ip internal command for the lan
dataForLan=$(ip a | awk '/: e/{gsub(/:/,"");print $2}')
#lan internal ip command
myLanIP=$(ip a s $dataForLan|awk '/inet /{gsub(/\/.*/,"");print $2}')
#lan hostname internal command 1
dataForLanHostname1=$(ip a | awk '/: e/{gsub(/:/,"");print $2}')
#lan ostname internal command 2
dataForLanHostname2=$(ip a s $dataForLanHostName1|awk '/inet /{gsub(/\/.*/,"");print $2}')
# ther is assign lan host name
myLanHostname=$(getent hosts $dataForLanHostname2| awk '{print $2}')
# it is command for external ip address
myExternalIP=$(curl -s icanhazip.com)
#it is for external ip address
myExternalName=$(getent hosts $myExternalIP | awk '{print $2}')
# Task 2: Add variables for the default router's name and IP address.
#         Add a name for the router's IP address to your /etc/hosts file.
#         The router's name and address must be obtained by dynamically
...
#         up the router's hostname using its IP address, not by just
#         printing out literal text.
myRouterIP=$(ip r s default | cut -d ' ' -f 3)
#it is give router name
myRouterName=$(getent hosts $myRouterIP| awk '{print $2}')
...
External IP     : $myExternalIP
External Name   : $myExternalName
Router Address  : $myRouterIP
Router HostName : $myRouterName
EOF
